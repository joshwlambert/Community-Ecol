...
oceanic_time_lac_mu <- dplyr::mutate(oceanic_time_lac_mu, log(lambda_a))

#log transform lambda_c_sim
oceanic_time_lac_mu <- dplyr::mutate(oceanic_time_lac_mu, log(lambda_a_sim))

#calculate absolute difference between ML estimate and true value
oceanic_abs_diff <- dplyr::mutate(oceanic_time_lac_mu, abs(log(lambda_a) - log(lambda_a_sim)))
...
oceanic_mean_laa <- mean(oceanic_abs_diff$`abs(log(lambda_a) - log(lambda_a_sim))`)
...
#calculate the average of the absolute differences
nonoceanic_mean_laa <- mean(nonoceanic_abs_diff$delta_lam_a)
