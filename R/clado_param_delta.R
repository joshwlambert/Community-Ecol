...
oceanic_time_lac_mu <- dplyr::mutate(oceanic_time_lac_mu, log(lambda_c))

#log transform lambda_c_sim
oceanic_time_lac_mu <- dplyr::mutate(oceanic_time_lac_mu, log(lambda_c_sim))
...
oceanic_mean_lac <- mean(oceanic_abs_diff$`abs(log(lambda_c) - log(lambda_c_sim))`)
...
#log transform lambda_c
nonoceanic_time_lac_mu <- dplyr::mutate(nonoceanic_time_lac_mu, log_lambda_c = log(lambda_c))

#log transform lambda_c_sim
nonoceanic_time_lac_mu <- dplyr::mutate(nonoceanic_time_lac_mu, log(lambda_c_sim))

#calculate the absolute difference between ML estimate and true value
nonoceanic_abs_diff <- dplyr::mutate(nonoceanic_time_lac_mu_mainland_nonend, delta_lam_c = abs(log(lambda_c) - log(lambda_c_sim)))

#remove any NA values
nonoceanic_abs_diff <- tidyr::drop_na(nonoceanic_abs_diff)

#calculate the average of the absolute differences
nonoceanic_mean_lac <- mean(nonoceanic_abs_diff$`abs(log(lambda_c) - log(lambda_c_sim))`)
